...
const CategoryCard = ({ icon, title, desc, onClick }: { icon: any, title: string, desc: string, onClick: () => void }) => (
  <button onClick={onClick} className="group text-left p-5 bg-white rounded-xl border border-slate-100 hover:border-slate-200 hover:shadow-lg transition-all duration-300">
    <div className="w-10 h-10 bg-slate-50 text-slate-500 rounded-lg flex items-center justify-center mb-3 group-hover:bg-kz-blue group-hover:text-white transition-colors duration-300">
      {icon}
    </div>
    <h3 className="font-semibold text-base text-slate-800 mb-0.5">{title}</h3>
...
const StepCard = ({ number, title, desc }: { number: string, title: string, desc: string }) => (
  <div className="flex gap-4 group">
    <div className="flex-shrink-0">
      <div className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center text-sm font-bold text-slate-500 group-hover:bg-kz-blue group-hover:text-white transition-colors duration-300">
        {number}
      </div>
    </div>
    <div className="pt-1">
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans text-slate-900">
      ...
      <main className="flex-1">
        {view === 'HOME' && (
          <>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 lg:py-24">
              {/* Categories */}
              <div className="mb-20">
                <h2 className="text-2xl font-bold text-slate-800 mb-1">Кем вы хотите стать?</h2>
                <p className="text-slate-400 mb-8 text-sm">Выберите направление, чтобы увидеть лучшие программы</p>
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                  <CategoryCard icon={<Laptop className="w-5 h-5" />} title="IT" desc="Программирование, ИИ" onClick={goCatalog} />
                  <CategoryCard icon={<Stethoscope className="w-5 h-5" />} title="Медицина" desc="Лечебное дело" onClick={goCatalog} />
                  <CategoryCard icon={<Coins className="w-5 h-5" />} title="Экономика" desc="Финансы, Учет" onClick={goCatalog} />
                  <CategoryCard icon={<Hammer className="w-5 h-5" />} title="Инженерия" desc="Строительство" onClick={goCatalog} />
                  <CategoryCard icon={<PenTool className="w-5 h-5" />} title="Дизайн" desc="Архитектура" onClick={goCatalog} />
                  <CategoryCard icon={<ScaleIcon className="w-5 h-5" />} title="Право" desc="Юриспруденция" onClick={goCatalog} />
                  <CategoryCard icon={<BookOpen className="w-5 h-5" />} title="Педагогика" desc="Учителя, Психология" onClick={goCatalog} />
                  <CategoryCard icon={<Atom className="w-5 h-5" />} title="Науки" desc="Химия, Биология" onClick={goCatalog} />
                </div>
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-12 items-center mb-24">
                <div>
                  <h2 className="text-2xl font-bold text-slate-800 mb-3">Как поступить на грант?</h2>
                  <p className="text-slate-500 text-sm mb-8 leading-relaxed">
                    Мы систематизировали процесс выбора вуза, чтобы максимизировать ваши шансы.
                  </p>
                  <div className="space-y-5">
                    <StepCard number="01" title="Выберите предметы" desc="Укажите ваши предметы ЕНТ и примерный балл." />
                    <StepCard number="02" title="Сравните шансы" desc="Система покажет вузы с высокой вероятностью." />
                    <StepCard number="03" title="Изучите вуз" desc="Посмотрите 3D-туры и отзывы." />
                  </div>
                  <button onClick={goAi} className="mt-8 bg-kz-blue text-white px-6 py-2.5 rounded-lg text-sm font-medium hover:bg-cyan-600 transition-colors shadow-lg shadow-cyan-500/20">
                    Попробовать
                  </button>
                </div>
                <div className="relative">
                  <div className="bg-white p-6 rounded-2xl shadow-lg border border-slate-100">
                    ...
                    <p className="mt-4 text-center text-xs text-slate-400">
                      *Данные обновляются на основе статистики МОН РК
                    </p>
                  </div>
                </div>
              </div>
        ...
        {view === 'AI_CHAT' && (
          <div className="overflow-hidden flex flex-col pt-4" style={{ height: 'calc(100vh - 100px)' }}>
            <h1 className="text-2xl font-bold mb-2 text-center text-slate-800">Умный помощник DataHub</h1>
      ...
      {/* Footer - Hide on AI Chat to optimize space */}
      {view !== 'AI_CHAT' && (
